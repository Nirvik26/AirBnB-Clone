...
function Home(props: hotelProp) {
  ...
  return (
    <div className="grid grid-cols-4">
      {props.hotels
        ?.filter((data: any) => data.name.includes(props.menu))
        .map((data: any) => {
          return (
            <>
              <Link
                onClick={() => addHotels(data)}
                to="/details"
                state={{ data: data }}
              >
                <div className="max-w-sm rounded-t-xl overflow-hidden w-72 h-11/12 ml-11 mt-7">
                  <img
